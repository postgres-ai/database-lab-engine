...
sed -ri 's/^(\s*)(PGUSER:.*$)/\1PGUSER: "'"${SOURCE_USERNAME}"'"/' ~/.dblab/server_test.yml
sed -ri 's/^(\s*)(PGPASSWORD:.*$)/\1PGPASSWORD: "'"${SOURCE_PASSWORD}"'"/' ~/.dblab/server_test.yml
sed -ri 's/^(\s*)(PGHOST:.*$)/\1PGHOST: "'"${SOURCE_HOST}"'"/' ~/.dblab/server_test.yml
