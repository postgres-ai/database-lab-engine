...
- **Clone major upgrade** (`engine/internal/{provision,cloning,srv}/upgrade.go` + `scripts/pg_upgrade_clone.sh` + `Dockerfile.pg-upgrade`): `pg_upgrade --link` converts a clone's data directory in place. Four invariants hold the design together:
  ...
  - **The target major is the instance's, not the caller's.** `Dockerfile.pg-upgrade` takes `PG_MAJOR` from its base image, so one configured `provision.pgUpgradeImage` means exactly one reachable target; there is nothing for an API field to choose. `Provisioner.UpgradeTargetVersion` derives it — `probe.MajorFromImage` off the tag, falling back to `PG_MAJOR` in the image config (`docker.ImagePGMajor`) for a tag outside the release grammar, cached under the image it came from so a `Reload` that repoints `pgUpgradeImage` cannot be answered from the old entry. Only released images carry a bare-major tag (CI publishes `pg-upgrade:17-<branch>`), so `ResolveUpgradeTarget` fetches the image in the background from `main.go` and on `Reload`; without it the feature would sit unavailable on every non-released instance. `Server.cloneUpgradeStatus` advertises the result as `cloneUpgrade` on `/status` so clients display the version instead of asking for one, and `verifyUpgradeImageMajor` re-reads the image before the clone goes down — a tag only claims a major while the image states one — which also refreshes the cache. An inspect **error** there fails the upgrade rather than passing the guard: `docker.ImagePGMajor` returns `(0, nil)` only for an image that genuinely declares no `PG_MAJOR`.
