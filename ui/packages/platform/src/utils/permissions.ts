...
import { OrgPermissions } from '@postgres.ai/platform/src/components/types'
...
export default {
  userIsOwner: function (
    ...
    data: {
      orgs?: {
        [org: string]: { owner_user_id: number; id: number }
      }
    },
  ) {
    for (const i in data.orgs) {
      if (
        Object.prototype.hasOwnProperty.call(data.orgs, i) &&
        data.orgs[i].id === orgId &&
  ...
  isAdmin: function (orgData: { role: { id: number } }) {
  ...
  */
  getPermissions: function (orgData: { role: { permissions: string[] } }) {
    ...
    orgData.role.permissions.forEach((p: string) => {
