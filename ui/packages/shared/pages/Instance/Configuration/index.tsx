...
export const Configuration = observer(
  ({
    ...
    const isConfigurationDisabled =
      !isConfigurationActive || instance?.state.engine.disableConfigModification

    const [isModalOpen, setIsModalOpen] = useState(false)
    const [connectionMessage, setConnectionMessage] = useState('')
    const [dockerImages, setDockerImages] = useState<string[]>([])
    const [isConnectionLoading, setIsConnectionLoading] = useState(false)
    const [submitMessage, setSubmitMessage] = useState<React.ReactNode>('')
    ...
    const onSubmit = async (values: FormValues) => {
      setSubmitMessage('')
      await updateConfig(values).then((res) => {
        if (res?.ok) {
          setSubmitMessage(
            <p>
              Changes applied.{' '}
    ...
    const onTestConnectionClick = async () => {
      ...
      if (isConnectionDataValid) {
        ...
        testDbSource(connectionData).then((res) => {
    ...
    useEffect(() => {
      if (config) {
        for (const [key, value] of Object.entries(config)) {
          ...
          setDockerImages(formatDockerImageArray(config?.dockerImageType || ''))
        }
      }
    }, [config])
    ...
    return (
      <div className={styles.root}>
        ...
        <Box>
              ...
              <Box mt={1}>
                ...
                <Box mt={2}>
                  <Button
                    ...
                    isDisabled={isConnectionLoading || isConfigurationDisabled}
                  >
                    Test connection
                    {isConnectionLoading && (
                ...
                {connectionMessage || dbSourceError ? (
                  <ResponseMessage
                    type={dbSourceError ? 'error' : 'success'}
                    message={dbSourceError || connectionMessage}
                  />
                ) : null}
              </Box>
            </Box>
            ...
            {dleEdition !== 'community' && (
              <>
                <InputWithChip
                  ...
                  disabled={isConfigurationDisabled}
                  onChange={(e) =>
                    formik.setFieldValue('pgDumpCustomOptions', e.target.value)
                  }
                />
                <InputWithChip
          ...
          {submitMessage || configError ? (
            <ResponseMessage
              type={configError ? 'error' : 'success'}
