...
<div align="center">
  ...
  Thin-Klone von PostgreSQL zum Erstellen leistungsstarker Entwicklungs-, Test-, QA- und Staging-Umgebungen.<br>
  <sub>Verfügbar für jedes PostgreSQL, einschließlich AWS RDS, GCP CloudSQL, Heroku, Digital Ocean und selbstverwaltete Instanzen.</sub>
</div>
...
<div align="center">
  <h3>
    ...
    <span> | </span>
    <a href="https://postgres.ai/blog/tags/database-lab-engine">Bloggen</a>
    <span> | </span>
    <a href="#community--support">Gemeinschaft und Unterstützung</a>
    <span> | </span>
...
## Warum DLE?
- Erstellen Sie Entwicklungs-/QA-/Staging-Umgebungen basierend auf produktionsähnlichen Datenbanken in voller Größe.
- Stellen Sie temporäre Datenbankklone in voller Größe für die Analyse und Optimierung von SQL-Abfragen bereit (siehe auch: [SQL-Optimierungs-Chatbot Joe](https://gitlab.com/postgres-ai/joe)).
- Testen Sie automatisch Datenbankänderungen in CI/CD-Pipelines, um Vorfälle in der Produktion zu vermeiden.

Beispielsweise dauert das Klonen einer PostgreSQL-Datenbank mit 1 TiB ca. 10 Sekunden. Dutzende von unabhängigen Klonen laufen auf einem einzigen Computer und unterstützen viele Entwicklungs- und Testaktivitäten, ohne die Kosten für Hardware zu erhöhen.

<p><img src="../assets/dle-demo-animated.gif" border="0" /></p>

Probieren Sie es gleich selbst aus:
- Geben Sie [die Database Lab Platform] (https://console.postgres.ai/) ein, treten Sie der "Demo"-Organisation bei und testen Sie das Klonen von ~1 TiB-Demodatenbank, oder
- Sehen Sie sich ein anderes Demo-Setup an, DLE CE: https://nik-tf-test.aws.postgres.ai:446/instance, verwenden Sie das Token „demo“, um es einzugeben (dieses Setup hat selbstsignierte Zertifikate, ignorieren Sie also die des Browsers Beschwerden)

## Wie es funktioniert
Thin Cloning ist schnell, da es [Copy-on-Write (CoW)](https://en.wikipedia.org/wiki/Copy-on-write#In_computer_storage) verwendet. DLE unterstützt zwei Technologien, um CoW und Thin Cloning zu ermöglichen: [ZFS](https://en.wikipedia.org/wiki/ZFS) (Standard) und [LVM](https://en.wikipedia.org/wiki/Logical_Volume_Manager_ (Linux)).

Mit ZFS erstellt Database Lab Engine regelmäßig einen neuen Snapshot des Datenverzeichnisses und verwaltet eine Reihe von Snapshots, wobei alte und nicht verwendete Snapshots bereinigt werden. Beim Anfordern eines neuen Klons können Benutzer auswählen, welcher Snapshot verwendet werden soll.
...
- GitLab: [Wie GitLab den Arbeitsablauf zur SQL-Leistungsoptimierung durchläuft, um das Risiko von Ausfallzeiten zu reduzieren](https://postgres.ai/resources/case-studies/gitlab)

## Merkmale
- Blitzschnelles Klonen von Postgres-Datenbanken – ein paar Sekunden, um einen neuen Klon zu erstellen, der bereit ist, Verbindungen und Abfragen zu akzeptieren, unabhängig von der Datenbankgröße.
...
- UI, um die manuelle Arbeit bequemer zu machen.
